...
const deleteTables = async (baseName: string) => {
  try {
...
const getRole = async (restaurantName: string, token: string) => {
  const { data, error } = await supabase
    .from(restaurantName)
    .select("role")
    .eq("token", token)
...
const getName = async (restaurantName: string, token: string) => {
  const { data, error } = await supabase
    .from(restaurantName)
    .select("name")
    .eq("token", token)
...
const getDateToken = async (restaurantName: string, token: string) => {
  const { data, error } = await supabase
    .from(restaurantName)
    .select("tokenTime")
    .eq("token", token)
...
const boolIsRestaurant = async (restaurantName: string) => {
  try {
...
const boolUserExist = async (restaurantName: string, userName: string) => {
  try {
...
const deleteOrderDB = async (restaurantName: string, orderID: number) => {
  // try {
...
const deleteFromTable = async (
  ...
  if (!count) count = "*";
  await supabase.from(tableName).delete().eq(columnEqual, valueEqual);
};

const loadOrders = async (restaurantName: string) => {
  const { data, error } = await supabase
...
const updateTableByEq = async (
  restaurantName: string,
  dict: object,
  valueEqual: Exclude<any, null | undefined>,
  equalColumn: string
) => {
  try {
    const { error } = await supabase
...
const showColumnsFromTable = async (restaurantName: string) => {
  try {
    const { data, error } = await supabase
      .from(restaurantName)
      .select("*")
...
const insertInTable = async (tableName: string, dict: object) => {
...
const getAllDataFromTable = async (tableName: string) => {
  try {
...
const getAllDataFromTableByEq = async (
  tableName: string,
  equalColumn: string,
  valueEqual: Exclude<any, null | undefined>
) => {
